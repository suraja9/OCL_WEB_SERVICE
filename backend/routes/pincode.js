...
const findPincode = async (pincode) => {
  ...
  try {
    ...
    const records = await PinCodeArea.find({ 
      ...
      .select('cityname areaname distrcitname statename serviceable bulkOrder priority standard modes')
    ...
    if (records.length > 0) {
      ...
      const result = {
        ...
        modes: {
          byAir: records.some(record => record.modes?.byAir),
          byTrain: records.some(record => record.modes?.byTrain),
          byRoad: records.some(record => record.modes?.byRoad)
        }
      };
...
router.get("/:pin", async (req, res) => {
  ...
  try {
    ...
    return res.json({ 
      ...
      priorityAvailable: result.priorityAvailable,
      standardAvailable: result.standardAvailable,
      modes: result.modes,
