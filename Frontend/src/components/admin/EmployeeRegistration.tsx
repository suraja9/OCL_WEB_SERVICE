...
const EmployeeRegistration = () => {
  ...
  const [otherDocumentsPreview, setOtherDocumentsPreview] = useState<string[]>([]);
  ...
  // Note: Employee code is now the same as unique ID (OCL001, OCL002, etc.)

  // Generate unique ID starting from OCL0001 using database
  const generateUniqueId = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setUniqueId(response.data.nextId);
        return response.data.nextId;
      } else {
        throw new Error('Failed to get next employee ID');
      }
    } catch (error) {
      console.error('Error generating unique ID:', error);
      // Fallback to timestamp-based ID if API fails
      const timestamp = Date.now();
      const fallbackId = `OCL${String(timestamp).slice(-4)}`;
    ...
    if (formData.primeMobileNumbers[0] && formData.primeMobileNumbers[0].length === 10) {
      try {
        const response = await fetch('http://localhost:5000/api/otp/send', {
          ...
          body: JSON.stringify({
            phoneNumber: formData.primeMobileNumbers[0]
          })
        });
  ...
  const handleFormSubmit = async () => {
    ...
    if (!formData.phone?.trim() && (!formData.primeMobileNumbers || !formData.primeMobileNumbers[0]?.trim())) {
      toast({
        title: "Validation Error",
    ...
    // Note: birthday is not required in the backend model, so we don't validate it here
    ...
    if (!formData.qualification?.trim()) {
      toast({
        title: "Validation Error",
        description: "Please enter qualification.",
        variant: "destructive"
      });
      return;
    }
    ...
    if (!formData.addressType?.trim()) {
      toast({
        title: "Validation Error",
        description: "Please select address type.",
        variant: "destructive"
      });
      return;
    }

    // Validate address fields based on address type
    if (!formData.locality?.trim() || !formData.buildingFlatNo?.trim() || 
    ...
    // If address type is permanent, validate permanent address fields
    if (formData.addressType === 'permanent' && 
        (!formData.permanentLocality?.trim() || !formData.permanentBuildingFlatNo?.trim() || 
         !formData.permanentPincode?.trim() || !formData.permanentCity?.trim() || 
         !formData.permanentState?.trim())) {
      toast({
        title: "Validation Error",
        description: "Please fill all required permanent address fields.",
        variant: "destructive"
      });
      return;
    }

    // If address type is present and sameAsPresent is false, validate other present address fields
    if (formData.addressType === 'present' && !formData.sameAsPresent &&
        (!formData.otherPresentLocality?.trim() || !formData.otherPresentBuildingFlatNo?.trim() || 
         !formData.otherPresentPincode?.trim() || !formData.otherPresentCity?.trim() || 
         !formData.otherPresentState?.trim())) {
      toast({
        title: "Validation Error",
        description: "Please fill all required alternate present address fields.",
        variant: "destructive"
      });
      return;
    }
    ...
    const validReferences = formData.references.filter(ref => 
      ref.name?.trim() && ref.relation?.trim() && ref.mobile?.trim()
    );
    
    if (validReferences.length === 0) {
      toast({
        title: "Validation Error",
        description: "Please provide at least one complete reference.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      // Generate unique ID and use it as employee code
      const currentUniqueId = await generateUniqueId();
      const employeeCode = currentUniqueId; // Use the sequential ID as employee code
      ...
      // Prepare form data for submission - mapping frontend fields to backend expected fields
      ...
      const submissionData = {
        ...
        phone: formData.primeMobileNumbers[0] || formData.phone, // Use prime mobile as primary phone
        alternativePhone: formData.alternativePhone || formData.primeMobileNumbers[1] || '',
        dateOfBirth: formData.birthday ? new Date(formData.birthday).toISOString() : null,
        designation: formData.designation,
        aadharNo: formData.aadharNo.replace(/\s/g, ''), // Remove spaces from Aadhar
        ...
        permanentBuildingFlatNo: formData.permanentBuildingFlatNo,
        ...
        references: validReferences, // Use only valid references
        primeMobileNumbers: formData.primeMobileNumbers.filter(mobile => mobile.trim()) // Filter out empty mobile numbers
      };
      
      // Debug: Check if required fields are present
      console.log('Required field checks:', {
        hasDateOfBirth: !!formData.birthday,
        hasPanNo: !!formData.panNo?.trim(),
        hasQualification: !!formData.qualification?.trim(),
        hasPhone: !!(formData.primeMobileNumbers[0] || formData.phone)?.trim(),
        hasAddress: !!(formData.locality?.trim() && formData.buildingFlatNo?.trim() && 
                     formData.pincode?.trim() && formData.city?.trim() && formData.state?.trim()),
        hasBirthday: !!formData.birthday,
        birthdayValue: formData.birthday
      });
      ...
      const response = await axios.post('http://localhost:5000/api/employee/register', submissionData, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        }
      ...
      if (response && response.data && response.data.success) {
        ...
        toast({
          ...
          description: `Employee registered successfully with ID: ${employeeCode}`,
        });
      } else {
        throw new Error(response?.data?.error || 'Failed to register employee');
      }
    } catch (error: any) {
      ...
      if (error.response) {
        ...
        if (error.response.status === 401) {
          ...
          if (error.response.data?.details) {
            ...
            errorMessage = `Server validation error: ${error.response.status}`;
          }
        } else {
  ...
  const handleNewForm = () => {
    ...
    // Generate new unique ID for the next employee
    generateUniqueId();
  };
  ...
  const lookupPincode = async (pincode: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Show success message for found pincode
        toast({
          title: "Pincode Found",
  ...
  const handleOtherDocumentsChange = (files: FileList | null) => {
    if (files) {
      ...
      newFiles.forEach(file => {
        ...
        reader.onload = (e) => {
          const result = e.target?.result as string;
          setOtherDocumentsPreview(prev => [...prev, result]);
        };
        reader.readAsDataURL(file);
      });
    }
  ...
  const removeOtherDocument = (index: number) => {
    ...
    setOtherDocumentsPreview(prev => prev.filter((_, i) => i !== index));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Generate unique ID and use it as employee code
      const currentUniqueId = await generateUniqueId();
      const employeeCode = currentUniqueId;
            ...
            <div className="text-center">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <p className="text-lg font-semibold text-blue-800">
                  Employee ID: <span className="underline">{uniqueId}</span>
                </p>
              </div>
              <Button
                onClick={() => {
              ...
              <div className="space-y-6 animate-in fade-in-50 duration-500">
                ...
                <div>
                  <FloatingLabelInput
                    ...
                    onChange={(value) => setFormData(prev => ({ ...prev, panNo: value.toUpperCase() }))}
                    placeholder="PAN No"
                    maxLength={10}
                    error={errors.panNo}
                    required
                ...
                {/* Others Documents Upload Section */}
                <div className="mt-6 rounded-lg p-4 border border-gray-200 shadow-md hover:shadow-lg transition-shadow duration-200" style={{ backgroundColor: '#f8f9fa' }}>
                  <h3 className="text-sm font-semibold text-gray-700 mb-4">Other Documents (Optional)</h3>
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors bg-white">
                    <input
                      ...
                      multiple
                      onChange={(e) => handleOtherDocumentsChange(e.target.files)}
                      className="hidden"
                    />
                    <label htmlFor="otherDocuments" className="cursor-pointer">
                      <div className="flex flex-col items-center">
                        <Upload className="h-8 w-8 text-gray-400 mx-auto mb-2" />
                        <p className="text-sm text-gray-600 mb-1">Click to upload multiple documents</p>
                        <p className="text-xs text-gray-500">PDF, JPG, PNG, DOC, DOCX files supported</p>
                      </div>
                    </label>
                  </div>
                  ...
                  {formData.otherDocuments.length > 0 && (
                    <div className="mt-4 space-y-2">
                      ...
                      {formData.otherDocuments.map((file, index) => (
                        <div key={index} className="flex items-center justify-between bg-white border border-gray-200 rounded p-3">
                          <div className="flex items-center gap-2 text-sm text-gray-700">
                            <FileText className="h-4 w-4" />
                          ...
                          <div className="flex items-center gap-2">
                            <button 
                              ...
                              onClick={() => { setViewerUrl(otherDocumentsPreview[index]); setShowViewer(true); }}
                            >
                              <Eye className="h-4 w-4" />
                            </button>
                            <button 
