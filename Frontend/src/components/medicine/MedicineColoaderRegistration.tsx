...
const MedicineColoaderRegistration = () => {
                  ...
                  <div className="space-y-3">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        onClick={() => addFromVehicleDetail(location.id)}
                    ...
                    <div className="space-y-3">
                      {location.vehicleDetails.map((detail) => (
                        <div key={detail.id} className="space-y-3 rounded-lg border border-gray-200 p-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FloatingLabelInput
                              id={`from-vehicle-name-${location.id}-${detail.id}`}
                              value={detail.vehicleName}
                              onChange={(value) => handleFromVehicleDetailChange(location.id, detail.id, 'vehicleName', value)}
                              placeholder="Vehicle Name"
                              icon={<Truck className="h-4 w-4" />}
                            />
                            <FloatingLabelInput
                              id={`from-vehicle-number-${location.id}-${detail.id}`}
                              value={detail.vehicleNumber}
                              onChange={(value) => handleFromVehicleDetailChange(location.id, detail.id, 'vehicleNumber', value)}
                              placeholder="Vehicle Number"
                              icon={<Truck className="h-4 w-4" />}
                              className="uppercase"
                            />
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FloatingLabelInput
                              id={`from-driver-name-${location.id}-${detail.id}`}
                              value={detail.driverName}
                              onChange={(value) => handleFromVehicleDetailChange(location.id, detail.id, 'driverName', value)}
                              placeholder="Driver Name"
                              icon={<User className="h-4 w-4" />}
                            />
                            <FloatingLabelInput
                              id={`from-driver-number-${location.id}-${detail.id}`}
                              value={detail.driverNumber}
                              onChange={(value) => handleFromVehicleDetailChange(location.id, detail.id, 'driverNumber', value)}
                              placeholder="Driver Contact Number"
                              type="tel"
                              maxLength={15}
                              icon={<Phone className="h-4 w-4" />}
                          ...
                          {location.vehicleDetails.length > 1 && (
                            <div className="flex justify-end">
                              <Button
                                ...
                                variant="outline"
                                size="sm"
                                onClick={() => removeFromVehicleDetail(location.id, detail.id)}
                                className="text-red-600 hover:text-red-700 hover:bg-red-50"
                              >
                                <Trash2 className="h-4 w-4" />
                              </Button>
...
export default MedicineColoaderRegistration;
